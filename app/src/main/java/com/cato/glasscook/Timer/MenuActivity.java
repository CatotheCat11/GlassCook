...
public class MenuActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Bind to the Timer service to retrieve the current timer's data.
    ...
    public boolean onPreparePanel(int featureId, View view, Menu menu) {
        ...
        if (isMyMenu(featureId)) {
            if (mTimer == null) {
                ...
                setOptionsMenuState(
                        menu.findItem(R.id.open_recipe),
                        mTimer.getRemainingTimeMillis() > 0);
    ...
    public boolean onMenuItemSelected(int featureId, MenuItem item) {
        ...
        if (itemId == R.id.start || itemId == R.id.resume) {
            mTimer.start();
            return true;
            ...
            mTimer.reset();
            return true;
        } else if (itemId == R.id.open_recipe) {
            Intent timerIntent = new Intent(MenuActivity.this, TimerService.class);
            timerIntent.setAction(TimerService.ACTION_RECIPE);
            timerIntent.putExtra(TimerService.EXTRA_TIMER_HASH_CODE, mTimer.hashCode());
