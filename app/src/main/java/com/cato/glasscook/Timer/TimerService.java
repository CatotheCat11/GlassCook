...
package com.cato.glasscook.Timer;
...
import com.cato.glasscook.CookingActivity;
...
public class TimerService extends Service {
    ...
    public static final String ACTION_RECIPE = "com.google.android.glass.sample.timer.action.RECIPE";
    ...
    private void handleIntent(Intent intent) {
        ...
        if (action.equals(ACTION_START)) {
            ...
            if (intent.hasExtra("recipe") && intent.hasExtra("step")) {
                recipe = intent.getStringExtra("recipe");
                step = intent.getIntExtra("step", 0);
            }

            mManager.startNewTimer(durationMillis);
        } else if (action.equals(ACTION_RECIPE)) {
            if (mManager.stopTimer(intent.getIntExtra(EXTRA_TIMER_HASH_CODE, 0))) {
                Intent cookingIntent = new Intent(this, CookingActivity.class);
                cookingIntent.putExtra("recipe", recipe);
                cookingIntent.putExtra("step", step);
                cookingIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
